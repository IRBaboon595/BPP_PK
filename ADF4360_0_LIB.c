...
void ADF4360_init(ADF4360_InitTypeDef *phADF4360i, SPI_HandleTypeDef *hspi)
{
	long_std_union temp;
	temp.listd = 0;
	/*if(hspi == &hspi4)
	...
	SPI_BUFF[0] = temp.cstd[2];
	SPI_BUFF[1] = temp.cstd[1];
	SPI_BUFF[2] = temp.cstd[0];
	
	if(hspi == &hspi4)
	{
		HAL_GPIO_WritePin(ADF4360_1_LE_GPIO_Port, ADF4360_1_LE_Pin, GPIO_PIN_RESET);			
	}
	else if(hspi == &hspi1)
	{
		HAL_GPIO_WritePin(ADF4360_2_LE_GPIO_Port, ADF4360_2_LE_Pin, GPIO_PIN_RESET);	
	}

	HAL_SPI_Transmit(hspi, SPI_BUFF, 3, 1);
